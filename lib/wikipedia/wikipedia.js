...
async function scrapeWikipedia(query, lang = 'en', disable_type) {
    try {
        const url = `https://${lang}.wikipedia.org/wiki/${query.replace(/\s+/g, '_')}`
        const { data } = await axios.get(url);
        const $ = cheerio.load(data);
        var disType = (Array.isArray(disable_type) == true) ? disable_type : Array(disable_type)
        ...
        $('#mw-content-text .mw-parser-output').children().each((index, element) => {
            ...
            if (tagName === 'P' && disType.indexOf('paragraph') == -1) {
                ...
                if (text) {
                    content.push({ type: 'other', content: text });
                }
            }
        });
